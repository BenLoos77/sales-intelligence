...
ACCENT = "#FFFFFF"          # MONOLITH: Cover-Hintergrund + Text auf dunkler Box (weiss)
ACCENT_DEEP = "#0F0F0F"      # MONOLITH: Akzente/Labels auf hellem Grund (schwarz)
...
def build_cover_svg(c: dict, date_display: str, color: str) -> str:
    ...
    return (
        ...
        f'<text x="40" y="60" font-family="JetBrains Mono, monospace" font-size="11" fill="#0a0a0a" letter-spacing="3" font-weight="600">{eyebrow}</text>'
        f'<text x="40" y="185" font-family="Hanken Grotesk, sans-serif" font-style="italic" font-weight="900" font-size="90" fill="#0a0a0a" letter-spacing="-3">{svg_text(c["kpi"])}</text>'
        f'<text x="40" y="220" font-family="JetBrains Mono, monospace" font-size="13" fill="#0a0a0a" letter-spacing="2">{svg_text(c["caps_line"])}</text>'
        '<line x1="40" y1="248" x2="560" y2="248" stroke="#0a0a0a" opacity="0.2"/>'
        '<line x1="40" y1="248" x2="120" y2="248" stroke="#0a0a0a" stroke-width="2"/>'
        f'<text x="40" y="293" font-family="Hanken Grotesk, sans-serif" font-style="italic" font-size="22" fill="#0a0a0a">{svg_text(c["italic1"])}</text>'
        f'<text x="40" y="321" font-family="Hanken Grotesk, sans-serif" font-style="italic" font-size="22" fill="#0a0a0a">{svg_text(c["italic2"])}</text>'
        f'<rect x="40" y="338" width="20" height="20" fill="{ACCENT_DEEP}"/>'
        f'<text x="72" y="354" font-family="JetBrains Mono, monospace" font-size="11" fill="#0a0a0a" letter-spacing="3" font-weight="600">{svg_text(c["bottom_caps"])}</text>'
        "</svg>"
    )
...
def build_hero_svg(h: dict, gid: str, color: str) -> str:
    ...
    return (
        ...
        f'  <text x="80" y="110" font-family="JetBrains Mono, monospace" font-size="13" fill="#0a0a0a" letter-spacing="6" font-weight="600">{svg_text(h["top_caps"])}</text>\n'
        f'  <text x="80" y="320" font-family="Hanken Grotesk, sans-serif" font-style="italic" font-weight="400" font-size="{big_size}" fill="#0a0a0a" letter-spacing="-8">{svg_text(big)}</text>\n'
        f'  <text x="{units_x}" y="320" font-family="Hanken Grotesk, sans-serif" font-style="italic" font-weight="400" font-size="150" fill="#0a0a0a" letter-spacing="-4">{svg_text(h.get("big_units", "/ 100"))}</text>\n'
        f'  <text x="80" y="372" font-family="JetBrains Mono, monospace" font-size="13" fill="#0a0a0a" letter-spacing="3" font-weight="600">{svg_text(h["caption_caps"])}</text>\n'
        '  <line x1="80" y1="414" x2="1200" y2="414" stroke="#0a0a0a" opacity="0.2"/>\n'
        '  <line x1="80" y1="414" x2="200" y2="414" stroke="#0a0a0a" stroke-width="2"/>\n'
        f'  <text x="80" y="472" font-family="Hanken Grotesk, sans-serif" font-style="italic" font-size="28" fill="#0a0a0a">{svg_text(h["italic1"])}</text>\n'
        f'  <text x="80" y="510" font-family="Hanken Grotesk, sans-serif" font-style="italic" font-size="28" fill="#0a0a0a">{svg_text(h["italic2"])}</text>\n'
        '  <rect x="80" y="559" width="430" height="108" fill="#0a0a0a"/>\n'
        f'  <text x="295" y="604" font-family="Hanken Grotesk, sans-serif" font-style="italic" font-size="36" fill="{ACCENT}" text-anchor="middle">{svg_text(h["box_value"])}</text>\n'
        f'  <text x="295" y="638" font-family="JetBrains Mono, monospace" font-size="11" fill="{ACCENT}" text-anchor="middle" letter-spacing="3" font-weight="600">{svg_text(h["box_caps"])}</text>\n'
        f'  <text x="560" y="588" font-family="Hanken Grotesk, sans-serif" font-style="italic" font-size="20" fill="#0a0a0a">{svg_text(h["right1"])}</text>\n'
        f'  <text x="560" y="618" font-family="Hanken Grotesk, sans-serif" font-style="italic" font-size="20" fill="#0a0a0a">{svg_text(h["right2"])}</text>\n'
        f'  <text x="560" y="648" font-family="Hanken Grotesk, sans-serif" font-style="italic" font-size="20" fill="#0a0a0a">{svg_text(h.get("right3", ""))}</text>\n'
        f'  <text x="80" y="703" font-family="JetBrains Mono, monospace" font-size="13" fill="#0a0a0a" letter-spacing="3" font-weight="500">{svg_text(h["source_caps"])}</text>\n'
        "</svg>"
    )
...
def build_inline_fig(fig: dict) -> str:
    ...
    parts = [
        ...
        f'        <text x="0" y="18" font-family="JetBrains Mono, monospace" font-size="12" fill="{ACCENT_DEEP}" letter-spacing="3" font-weight="600">{svg_text(fig.get("title_caps", ""))}</text>',
    ...
    for i, col in enumerate(cols):
        x = [0, 360, 720][i]
        parts.append(f'        <text x="{x}" y="138" font-family="Hanken Grotesk, sans-serif" font-style="italic" font-size="80" fill="#0a0a0a">{svg_text(col.get("num", ""))}</text>')
        parts.append(f'        <text x="{x}" y="178" font-family="Hanken Grotesk, sans-serif" font-size="21" fill="#0a0a0a">{svg_text(col.get("head", ""))}</text>')
        for ly, line in zip(line_ys, col.get("lines", [])[:4]):
            parts.append(f'        <text x="{x}" y="{ly}" font-family="Hanken Grotesk, sans-serif" font-size="15" fill="#C3CCD2">{svg_text(line)}</text>')
    parts.append("      </svg>")
    if fig.get("cap"):
      ...
      <div class="cover-inner">
        ...
        <div class="issue-title">
          <svg class="curved-title" viewBox="0 0 360 360" xmlns="http://www.w3.org/2000/svg">
            ...
            <text fill="#0F0F0F" font-family="Hanken Grotesk, sans-serif" font-size="24" font-weight="400">
              <textPath href="#circle" startOffset="2%">{svg_text(cov["curved_title"])}</textPath>
            </text>
            <text x="180" y="160" font-family="Hanken Grotesk, sans-serif" font-style="italic" font-weight="400" font-size="46" fill="#0F0F0F" text-anchor="middle">{svg_text(cov["kpi"])}</text>
            <text x="180" y="200" font-family="Hanken Grotesk, sans-serif" font-style="italic" font-weight="400" font-size="20" fill="#0F0F0F" text-anchor="middle">{svg_text(cov["subtitle"])}</text>
          </svg>
        </div>
...
<section class="page" id="page-schwerpunkt">
  ...
  <div class="article-hero">
    ...
    <div class="article-image" style="background: var(--paper);">
{hero_svg}
  </div>
...
ARTICLE_SCHEMA = {
    ...
    "properties": {
        ...
        "color": {"type": "string", "description": "Akzentfarbe Hex, Standard #FFFFFF"},
...
Akzentfarbe #FFFFFF. Gib alles über das Werkzeug submit_article aus."""
